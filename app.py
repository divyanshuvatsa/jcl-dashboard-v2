...
def render_sidebar():
    ...
    if uploaded:
        mode = st.sidebar.radio(
            ...
            ["Load full model (exact Excel numbers)", "Pull assumptions & re-simulate"],
            help=("'Load full model' reads the workbook's own computed figures and "
                  "displays them verbatim (fastest, 100% faithful). 'Pull assumptions' "
                  "feeds the Base drivers into the live engine so the sliders stay active."),
        )
        if st.sidebar.button("Apply", use_container_width=True):
            if mode.startswith("Load full model"):
                try:
                    with st.spinner("Loading workbook…"):
                        st.session_state.loaded_model = load_full_model(uploaded)
                    st.session_state.scenario = "Excel (loaded)"
                    st.sidebar.success("Loaded the workbook's exact computed model.")
                    st.rerun()
                except Exception as e:
                    st.sidebar.error(f"Could not load workbook: {e}")
            else:
    ...
    if st.session_state.get("loaded_model") is not None:
        if st.sidebar.button("Clear loaded workbook → back to live engine",
...
def main():
    ...
    if loaded is not None:
        results = loaded
        st.info("Showing the **exact computed model** loaded from the uploaded "
                "workbook (no re-simulation). Risk tools below use the live engine "
                "seeded with the workbook's active-scenario drivers.")
    else:
