...
├── engine.py             # Financial logic, calibrated to Excel (no Streamlit/network)
├── model_data.py         # Loads the bundled Base snapshot for the engine
├── jcl_base_model.json   # Golden Base snapshot extracted from the workbook
├── excel_loader.py       # Fast external full-model loader (exact Excel numbers)
├── verify.py             # V&V harness — 1,318 assertions vs the golden master
├── smoke_test.py         # End-to-end dashboard pipeline test
...
The split exists so each module can be unit-tested independently. The source
**workbook is the single source of truth**: `engine.py` reproduces it at Base
and flexes from there, `excel_loader.py` can display it verbatim, and
`verify.py` proves the two stay consistent.
...
The dashboard is now **calibrated to the source workbook to full numerical
precision**. `engine.py` is a digital twin of `JCL_Financial_Model_EXP.xlsx`:
the exogenous schedules the workbook computes on dedicated sheets
(depreciation, the interest cascade, debt & covenant schedules, the FY24A/FY25A
actuals, balance-sheet roll-forwards, the dividend profile, working-capital
days) are loaded verbatim from a bundled Base snapshot, and the
driver-sensitive lines (production, revenue, COGS, EBITDA, WACC, FCFF, DCF,
covenant ratios) are computed by formula on top. At the Base preset every
figure reproduces the workbook exactly; moving a slider perturbs the model
continuously from that exact base.

| Metric | Engine (Base) | Source Excel | Result |
|---|---|---|---|
| Value per share | ₹514.75 | ₹514.75 | **exact** |
| WACC | 12.51% | 12.51% | **exact** (D/E-weighted) |
| Enterprise value | ₹2,114.25 Cr | ₹2,114.25 Cr | **exact** |
| Terminal value | ₹2,099.46 Cr | ₹2,099.46 Cr | **exact** (CIT 25.17%) |
| PV of terminal value | ₹817.42 Cr | ₹817.42 Cr | **exact** |
| Sum of PV(FCFF) | ₹1,296.83 Cr | ₹1,296.83 Cr | **exact** |
| % EV from TV | 38.66% | 38.66% | **exact** |
| Min DSCR | 1.3531x (FY28E) | 1.3531x (FY28E) | **exact** |
| FY26E revenue | ₹2,081 Cr | ₹2,081 Cr | **exact** |
| Balance sheet | balances all 10 years | balances | **exact** |

### Verification & validation

`verify.py` is an automated harness that rebuilds the engine at Base **and**
loads the workbook through `excel_loader.py`, then compares both against the
golden snapshot cell-by-cell — **1,318 assertions per run** spanning every
income-statement, balance-sheet and cash-flow line for all ten years, the
revenue build, FCFF/NOPAT/PV series, all DCF and WACC components, the EV→equity
bridge, ROE/ROCE/current-ratio/D-E (cross-checked against the workbook's own
Ratio Analysis sheet), DSCR/ISCR for every covenant year, and the full 5×5
WACC×g sensitivity grid. Tolerances are ₹0.02 Cr on levels and 2e-4 relative on
ratios. Run it any time:

```bash
python verify.py        # exits 0 on a clean pass, non-zero on any mismatch
```

A full end-to-end check of the Streamlit pipeline (all three scenarios, every
chart, the Smart Analyst, the Excel export, the external loader) lives in
`smoke_test.py`.

### Loading a workbook externally is fast

The "Sync from Excel" panel can load an uploaded workbook's **exact computed
model** and display it verbatim. The loader reads cached cell values in a
single pass (`openpyxl`, `data_only=True`) with no formula recalculation, so a
~300 KB workbook loads in roughly half a second — there is no multi-second
office-engine recalculation step.
...
**Excel sync**
The "Sync from Excel" panel offers two modes. *Load full model* reads the
workbook's own computed numbers and displays them verbatim (fastest, exact) —
use this when you want to see the source model unchanged. *Pull assumptions &
re-simulate* feeds the Base drivers into the live engine so the sliders stay
active. The assumptions parser looks for a sheet named "Scenario Engine" (or
"Scenario_Engine", "Scenarios", "Drivers", "Assumptions") with a header row
containing "Bull", "Base", "Bear" in adjacent cells; if you renamed the sheet,
rename it back or edit `LABEL_MAP` in `engine.py`. Use "Clear loaded workbook"
to return to the live engine.
...
- **Digital-twin design.** `engine.py` does not re-derive the workbook from
  scratch. Exogenous schedules (depreciation, interest cascade, debt/covenant
  schedules, FY24A/FY25A actuals, BS roll-forward items, dividend profile,
  WC-days) are loaded verbatim from `jcl_base_model.json` via `model_data.py`;
  only the genuinely driver-sensitive lines are computed by formula. This is
  why Base is exact to the rupee while sliders still move everything downstream.
- **WACC weighting** follows the workbook precisely: `We = 1/(1+D/E)`, with the
  debt/preference split applied to the remaining `D/E/(1+D/E)`. The cost of
  equity uses a relevered beta `βL = βu·(1+(1−tax)·D/E)`. This reproduces
  12.51% exactly and responds correctly to the Target D/E slider.
- **Regenerating the snapshot.** If the source workbook changes, refresh the
  bundled Base with `excel_loader.regenerate_base_model(path)` (or re-run
  `extract_golden.py`), then run `verify.py` to confirm a clean pass.
- `engine.py` has zero Streamlit imports and zero network calls. Unit-test it
  standalone.
